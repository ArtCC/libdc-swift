...
dc_descriptor_t* get_descriptor(const char* vendor, const char* product) {
    for (size_t i = 0; i < C_ARRAY_SIZE(g_descriptors); i++) {
        if (strcasecmp(g_descriptors[i].vendor, vendor) == 0 &&
            strcasecmp(g_descriptors[i].product, product) == 0) {

...
suunto_eonsteel_receive_ble(suunto_eonsteel_device_t *device, unsigned char data[], unsigned int size, unsigned int *actual)
{
    ...
    printf("DEBUG: Entering suunto_eonsteel_receive_ble\n");

    rc = dc_iostream_read(device->iostream, buffer, sizeof(buffer), &transferred);
    if (rc != DC_STATUS_SUCCESS) {
        ERROR(device->base.context, "Failed to receive the packet.");
        printf("DEBUG: Failed to receive packet. Status: %d\n", rc);
        return rc;
    }
    ...
    if (nbytes > size) {
        ERROR(device->base.context, "Insufficient buffer space available.");
        printf("DEBUG: Buffer overflow. Required: %u, Available: %u\n", nbytes, size);
        return DC_STATUS_PROTOCOL;
    }

    memcpy(data, buffer, nbytes);
    printf("DEBUG: Copied %u bytes to output buffer\n", nbytes);
    ...
    printf("DEBUG: Exiting suunto_eonsteel_receive_ble successfully\n");
    return DC_STATUS_SUCCESS;
}
    ...
    unsigned int size)
{
    ...
    printf("DEBUG: Entering suunto_eonsteel_send\n");
    printf("DEBUG: Command: %04x, Size: %u\n", cmd, size);

    // Two-byte packet header, followed by 12 bytes of extended header
    if (size + 2 + HEADER_SIZE + CRC_SIZE > sizeof(buf)) {
        ERROR(device->base.context, "Insufficient buffer space available.");
        printf("DEBUG: Buffer overflow in suunto_eonsteel_send\n");
        return DC_STATUS_PROTOCOL;
    }
